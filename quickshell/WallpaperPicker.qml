...
Scope {
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                root.currentWallpaper =
                    path
                root.focusCurrentWallpaper()
                // Keep ThemeManager's Auto (matugen) mode in sync
                // even if the wallpaper was changed outside the
                // picker (e.g. by another tool) between sessions.
                ThemeManager.onWallpaperChanged(path)
            }
        }
        running:
            false
    ...
    function applyWallpaper(path) {
        ...
        root.currentWallpaper =
            path
        // If Auto (matugen) mode is on, this regenerates the
        // "wallpaper" palette and switches to it.
        ThemeManager.onWallpaperChanged(path)
    }
    // ═══════════════════════════════════════════════════════════════════
    // Window
